...
def encode(video_path: str, output_path: str) -> dict:
    ...
    global telemetry_bytes; telemetry_bytes = {i: 0 for i in range(16)}; telemetry_bytes[0xFF] = 0
    telemetry = {
        ...
        while True:
            ...
            for y_idx in range(grid_h):
                for x_idx in range(grid_w):
                    ...
                    if config.get('palette') and best_cost > 10:
                    ...
                    if best_cost > 30: 
                        ...
                        dither_sad = 0
                        if best_cost > 75 or (np.max(curr_block) - np.min(curr_block) > 40):
                            b_min, b_max = np.min(curr_block), np.max(curr_block)
    ...
    if total > 0:
        ...
        print(f"telemetry_dict_dump: {telemetry_bytes}")
    return {
