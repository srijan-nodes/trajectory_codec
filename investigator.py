...
def decode_and_investigate(original_video, encoded_path):
    print(f"\n🔍 Investigating {os.path.basename(original_video)} block-by-block...")
    ...
    with open(encoded_path, "rb") as f:
        ...
        while True:
            ...
            frame_modes = np.zeros((grid_h, grid_w), dtype=np.uint8) # Default is 0 (Skip)
            ...
            if frame_type == 0:
                ...
                frame_modes.fill(255) # Mark all blocks as I-Frame
                ...
                for y_idx in range(grid_h):
                    for x_idx in range(grid_w):
                        ...
                        # Log the mode used for this block region
            ...
            # 1. Calculate squared error for every pixel
            diff_sq = (orig_y - prev.squeeze().astype(np.float32)) ** 2
            
            # 2. Reshape into 16x16 blocks and calculate block-level MSE
            # diff_sq is shape (h, w). Reshape to (grid_h, 16, grid_w, 16)
            block_mse_grid = diff_sq.reshape(grid_h, BLOCK_SIZE, grid_w, BLOCK_SIZE).mean(axis=(1, 3))
            
            # 3. Accumulate stats grouped by mode
...
def main():
    ...
    video_path = filedialog.askopenfilename(
        title="Select Video to Investigate (.mp4)", 
    ...
    temp_nam = "temp_investigator.nam"
    
    # 1. Run the Encoder
    print(f"\nEncoding {os.path.basename(video_path)} via V5 Hybrid Engine...")
    try:
    ...
    # Sort stats by Average MSE (highest offenders at the bottom)
